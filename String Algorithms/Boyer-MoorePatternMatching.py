# -*- coding: utf-8 -*-
"""
Created on Sat Jun  5 21:47:41 2021

@author: ATA
"""

text = "acacabacababacacac"
pattern = "acacac"
...
while i<=len(text)-len(pattern):
    for j in range(len(pattern)-1, -1, -1):     #reverse searching
        if pattern[j] != text[i+j]: #miss-match location
            ...
            if j == len(pattern)-1:     #if good-suffix is not present, we test bad character 
                ...
                if len(text[i+j+k:i+len(pattern)]) != 1:    #good-suffix should not be of one character
                    gsshift=i+j+k-pattern[0:len(pattern)-1].rfind(text[i+j+k:i+len(pattern)]) #this line consider all possible good suffix cases in book as long as good-suffix is more than 1 character #jumps pattern to a position where good-suffix of pattern matches with good-suffix of text
                else:
